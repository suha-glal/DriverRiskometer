...
void startAcc()
	{
	TInt err;
	_LIT (KExeName, "c:\\sys\\bin\\AccelerationServer.exe");
...
void startCall()
	{
		...
		TInt err;_LIT (KExeName, "c:\\sys\\bin\\Calllog.exe");
		_LIT( KArguments, "-argument" );
		RProcess processHandle;
		 err = processHandle.Create(KExeName, KArguments);
		if (err == KErrNone) 
...
void startKey()
	...
	// Create a file to write the text to
		TInt err;_LIT (KExeName, "c:\\sys\\bin\\keys.exe");
		_LIT( KArguments, "-argument" );
		RProcess processHandle;
		 err = processHandle.Create(KExeName, KArguments);
		if (err == KErrNone) 
